...
files = glob.glob('../domain_surfdata/*', recursive=True)
for file in files:
    # Check if '_lnd.domain' is in the file name
    if '_domain.lnd.' in file:
        domain="domain.lnd.Daymet.km.1d.nc"
        # Only create the link if it does not already exist
        if not os.path.exists(domain):
            command = f'ln -s "{file}" "{domain}"'
            print(command)
            os.system(command)
        else:
            print(f"Link {domain} already exists, skipping.")
