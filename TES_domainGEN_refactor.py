...
def domain_save_1dTES(output_dir, domain_name, total_rows, total_cols, data, lon, lat, XC, YC, date_tag, verbose=False):
    ...
    # date_tag is provided by caller for deterministic outputs
    '''
    #Proj4: +proj=lcc +lon_0=-100 +lat_1=25 +lat_2=60 +k=1 +x_0=0 +y_0=0 +R=6378137 +f=298.257223563 +units=m  +no_defs
    geoxy_proj_str = "+proj=lcc +lon_0=-100 +lat_0=42.5 +lat_1=25 +lat_2=60 +x_0=0 +y_0=0 +R=6378137 +f=298.257223563 +units=m +no_defs"
    geoxyProj = CRS.from_proj4(geoxy_proj_str)
    ...
    file_name = os.path.join(output_dir, 'domain.lnd.' + domain_name + '.4km.1d.c' + date_tag + '.nc')
...
def domain_save_2dTES(output_dir, domain_name, total_rows, total_cols, data, lon, lat, XC, YC, date_tag, verbose=False):
    ...
    os.makedirs(output_dir, exist_ok=True)
    file_name = os.path.join(output_dir, 'domain.lnd.' + domain_name + '.4km.2d.c' + date_tag + '.nc')
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Generate 1D and 2D TESSFA domain NetCDF files from a forcing NetCDF",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("legacy", nargs="*", help=argparse.SUPPRESS)
    parser.add_argument("-i", "--input", help="Path to input forcing NetCDF file")
    parser.add_argument("-o", "--output-dir", help="Directory for generated domain files")
    parser.add_argument("-n", "--domain-name", help="Domain name token used in output filenames")
    parser.add_argument("--var-name", default="TBOT", help="Variable name used to derive land mask")
    parser.add_argument("--timesteps", type=int, default=1, help="Number of timesteps to read for mask computation")
    parser.add_argument("--date-tag", help="Override date tag (yymmdd) in output filenames")
    parser.add_argument("--only-1d", action="store_true", help="Generate only 1D domain file")
    parser.add_argument("--only-2d", action="store_true", help="Generate only 2D domain file")
    parser.add_argument("-v", "--verbose", action="store_true", help="Verbose logging")
    return parser.parse_args()


def _resolve_inputs(args):
    # Support legacy positional usage: <input_path> <input_filename> <output_path> <domain_name>
    if args.input and args.output_dir and args.domain_name:
        return args.input, args.output_dir, args.domain_name

    if len(args.legacy) == 4 and not args.input and not args.output_dir and not args.domain_name:
        input_path, input_filename, output_path, domain_name = args.legacy
        if not input_path.endswith("/"):
            input_path = input_path + "/"
        if not output_path.endswith("/"):
            output_path = output_path + "/"
        print("[DEPRECATED] Positional usage is deprecated. Use --input, --output-dir, --domain-name.")
        return input_path + input_filename, output_path, domain_name

    raise SystemExit("Error: provide --input, --output-dir, --domain-name (or legacy 4 positionals). Use -h for help.")


def _compute_date_tag(override):
    return override if override else datetime.now().strftime('%y%m%d')


def main():
    args = _parse_args()

    input_file, output_dir, domain_name = _resolve_inputs(args)

    os.makedirs(output_dir, exist_ok=True)

    timesteps = max(1, int(args.timesteps))
    var_name = args.var_name
    verbose = args.verbose

    start = process_time()
    with nc.Dataset(input_file, 'r', format='NETCDF4') as src:
        if 'lon' not in src.variables or 'lat' not in src.variables:
            raise SystemExit("Error: Input file must contain 'lon' and 'lat' variables.")
        if var_name not in src.variables:
            raise SystemExit(f"Error: Variable '{var_name}' not found in input file.")

        lon = src['lon'][:]
        lat = src['lat'][:]
        total_cols = lon.shape[0]
        total_rows = lat.shape[0]
        XC, YC = np.meshgrid(lon, lat)
        data = src[var_name][0:timesteps, :, :]
    ...
    if verbose:
        print("Reading input takes {}".format(end-start))

    date_tag = _compute_date_tag(args.date_tag)

    if not args.only_1d or args.only_2d:
        start = process_time()
        domain_save_2dTES(output_dir, domain_name, total_rows, total_cols, data, lon, lat, XC, YC, date_tag, verbose)
        end = process_time()
        if verbose:
            print("Saving 2D domain data takes {}".format(end-start))

    if not args.only_2d or args.only_1d:
        start = process_time()
        domain_save_1dTES(output_dir, domain_name, total_rows, total_cols, data, lon, lat, XC, YC, date_tag, verbose)
        end = process_time()
        if verbose:
