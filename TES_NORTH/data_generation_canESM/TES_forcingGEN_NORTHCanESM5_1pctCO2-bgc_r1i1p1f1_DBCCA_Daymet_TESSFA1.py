...
DATASET_ID = "CanESM5_1pctCO2-bgc_r1i1p1f1_DBCCA_Daymet_TESSFA1"
ALLOWED_TOP_DIRS = set(["TPHWL3Hrly", "Solar3Hrly", "Precip3Hrly"])
...
def forcing_save_1dTES(input_path, file, var_name, period, time, output_path):
    ...
    new_filename = f"clmforc.{DATASET_ID}.{resolution}.1d.{var_name}.{period}.nc"
...
def main():
    ...
    if (len(args) not in (3,5)) or (args and args[0] == '--help'):
        print("Example use: python TES_forcingGEN_NORTHCanESM5_1pctCO2-bgc_r1i1p1f1_DBCCA_Daymet_TESSFA1.py <input_path> <output_path> <time steps> [start_year end_year]")
        ...
        print(" [start_year end_year]: optional year filter for files like YYYY-MM")
    ...
    output_path = args[1]
    time = int(args[2])
    start_year = int(args[3]) if len(args) == 5 else None
    end_year = int(args[4]) if len(args) == 5 else None
    ...
    for root, dirs, files in os.walk(input_path):
        ...
        if top_dir not in ALLOWED_TOP_DIRS:
            # prune traversal into disallowed branches (e.g., DBCCA_dy)
            dirs[:] = []
            continue
        for file in files:
            ...
            if file.endswith('.nc'):
                ...
                parts = file.split('.')
                # Source pattern: clmforc.<resolution_km>.<var>.<yyyy-mm>.nc
                var_name = parts[2]
                period = parts[3]
                if start_year is not None and end_year is not None:
                    yr = period_year(period)
                    if yr is not None and (yr < start_year or yr > end_year):
                        continue
